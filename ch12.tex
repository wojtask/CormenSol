...
\exercise %12.3-4
W~przypadku, gdy węzeł $z$ będący argumentem procedury \proc{Tree-Delete} ma dwóch synów, usuwanym węzłem jest następnik $z$.
Problem pojawia się, gdy pewna struktura danych zawiera wskaźnik do węzła $y$, ale usuwany jest węzeł $z$, będący poprzednikiem $y$.
Struktura ta może następnie odnosić się do $y$, zakładając, że nadal należy on do drzewa.
...
Rozwiązanie tego problemu polega na podmianie węzła $z$ przez $y$ tuż przed zakończeniem działania \proc{Tree-Delete} w~przypadku, gdy węzeł $z$ początkowo miał dwóch synów.
