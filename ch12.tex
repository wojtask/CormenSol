...
\]
Z~kolei zauważmy, że najniższy poziom pełnego drzewa binarnego składa się z~$\Omega\bigl(n-\sqrt{n\lg n}\bigr)$ węzłów, z~których każdy ma głębokość $\Omega(\lg n)$.
Stąd średnia głębokość węzła ograniczona jest od dołu przez
\[
	\frac{1}{n}\cdot\Omega\bigl(\bigl(n-\sqrt{n\lg n}\bigr)\lg n\bigr) = \frac{1}{n}\cdot\Omega(n\lg n) = \Omega(\lg n).
\]
...
Załóżmy, że w~drzewie binarnym o~$n$ węzłach i~wysokości $h$ średnią głębokością węzła jest $\Theta(\lg n)$.
