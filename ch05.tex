...
% Zauważmy, że dla $n=1$ stwierdzenie trywialnie zachodzi, zatem załóżmy, że $n>1$. Niech $A_{ij}$ oznacza zdarzenie, że elementy na pozycjach $i$ oraz $j$ w~tablicy $P$ są różne. Mamy wykazać prawdziwość nierówności
% \[
% 	\E\biggl(\sum_{i=1}^{n-1}\sum_{j=i+1}^nI_{ij}\biggr) \ge 1-\frac{1}{n},
% \]
% gdzie $I_{ij}$ jest zmienną losową wskaźnikową zdarzenia $A_{ij}$ oznaczającego, że elementy na pozycjach $i$ oraz $j$ w~tablicy $P$ są różne. Nierówność przyjmuje postać
% \[
% 	\sum_{i=1}^{n-1}\sum_{j=i+1}^n\Pr(A_{ij}) \ge 1-\frac{1}{n},
% \]
% dzięki liniowości wartości oczekiwanej i~własności zmiennych losowych wskaźnikowych. Zauważmy, że prawdopodobieństwo tego, by elementy $i$ oraz $j$ były równe, wynosi $1/n^3$, a~więc $A_{ij}$, czyli zdarzenie przeciwne do niego, zachodzi z~prawdopodobieństwem $1-1/n^3$. Mamy stąd
% \begin{align*}
% 	\sum_{i=1}^{n-1}\sum_{j=i+1}^n\left(1-\frac{1}{n^3}\right) &\ge 1-\frac{1}{n} \\
% 	\left(1-\frac{1}{n^3}\right)\sum_{i=1}^{n-1}\sum_{j=i+1}^n1 &\ge 1-\frac{1}{n} \\
% 	\left(1-\frac{1}{n^3}\right)\sum_{i=1}^{n-1}(n-i) &\ge 1-\frac{1}{n} \\
% 	\left(1-\frac{1}{n^3}\right)\sum_{i=1}^{n-1}i &\ge 1-\frac{1}{n} \\
% 	\left(1-\frac{1}{n^3}\right)\frac{n(n-1)}{2} &\ge 1-\frac{1}{n}.
% \end{align*}
...
\]
dla $i=1$, 2,~\dots,~$k$ jest zdarzeniem polegającym na tym, że wśród $k$ osób, tylko \twoparts{$i$}{ta} obchodzi urodziny dnia $r$. Obliczmy szanse zajścia ostatniego z~nich, wiedząc, że podobnie jak wcześniej, zachodzi $\Pr(A_i)=1-1/n$:
\[
