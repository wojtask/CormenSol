...
\exercise %10.4-5
W~procedurze wykorzystamy zmienną \id{direction} wyznaczającą ,,kierunek'', w~którym obecnie poruszamy się po drzewie, wypisując jego klucze. Zmienna ta będzie przyjmować jedną z~czterech wartości:
\begin{itemize}
...
\end{table}
Jeśli będziemy utrzymywać atrybut \id{tail} wskazujący na ogon listy, to operację \proc{Maximum} dla list posortowanych możemy wykonywać w~czasie stałym.
...
\note{W~tłumaczeniu w~pseudokodzie procedury \proc{Compact-List-Search} jest błąd -- w linijce~4 zamiast sprawdzania, czy\/ $\id{key}[j]<k$ powinno być sprawdzanie, czy\/ $\id{key}[j]\le k$.}
