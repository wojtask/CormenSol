...
\li	\While $\id{curr}\ne\const{nil}$
\li		\Do
			\If $\id{prev}=\attrib{\id{curr}}{p}$
\li				\Then
					\If $\attrib{\id{curr}}{left}\ne\const{nil}$
\li						\Then $\id{next}\gets\attrib{\id{curr}}{left}$
\li						\Else $\id{next}\gets\proc{Stackless-Inorder-Visit}(\id{curr})$
						\End
\li				\ElseIf $\id{prev}=\attrib{\id{curr}}{left}$
\li					\Then $\id{next}\gets\proc{Stackless-Inorder-Visit}(\id{curr})$
\li				\ElseNoIf $\id{next}\gets\attrib{\id{curr}}{p}$	
				\End
