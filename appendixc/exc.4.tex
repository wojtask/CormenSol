...
\exercise %C.4-5
Możemy wyznaczyć przybliżenie prawdopodobieństwa nieuzyskania żadnego sukcesu $b(0;n,1/n)$, badając jego graniczną wartość:
\[
	\lim_{n\to\infty}b(0;n,1/n) = \lim_{n\to\infty}(1-1/n)^n = 1/e.
\]
Skorzystano z~faktu, że dla dowolnej liczby rzeczywistej $a$, $\lim_{n\to\infty}(1+a/n)^n=e^a$.

Przybliżenie prawdopodobieństwa uzyskania dokładnie jednego sukcesu $b(1;n,1/n)$ otrzymujemy analogicznie:
\[
	\lim_{n\to\infty}b(1;n,1/n) = \lim_{n\to\infty}\frac{\bigl(1-1/n\bigr)^n}{1-1/n} = \frac{1/e}{1} = 1/e.
\]

\exercise %C.4-6
Obliczymy dwoma sposobami prawdopodobieństwo uzyskania przez profesorów równej liczby orłów.
W~pierwszym z~nich będziemy traktować wynik każdego doświadczenia jako $(2n)$\nbhyphen elementowy ciąg orłów i~reszek taki, że jego początkowych $n$ wyrazów oznacza wyniki uzyskane przez profesora Rosencrantza, a~$n$ końcowych -- wyniki profesora Guildensterna.
...
\exercise %C.4-9
Niech $S$ będzie przestrzenią zdarzeń złożoną ze wszystkich możliwych $n$\nbhyphen słów nad alfabetem $\{{\scriptstyle\rm P},{\scriptstyle\rm S}\}$, gdzie ${\scriptstyle\rm P}$ oznacza porażkę, a~${\scriptstyle\rm S}$ -- sukces.
Ciągowi prób $A$ odpowiada zatem pewne słowo $s\in S$, przy czym $X(s)$ to liczba wystąpień ${\scriptstyle\rm S}$ w~$s$.

Zobaczymy, jak uzyskać ciąg prób Bernoulliego $A'$ przez wykonanie doświadczeń na próbach z~ciągu $A$.
Przez $A_i$ oznaczymy zdarzenie polegające na wystąpieniu sukcesu w~$i$\nbhyphen tej próbie ciągu $A$, a~przez $A_i'$ -- analogiczne zdarzenie dla ciągu $A'$.
Jeśli zachodzi $A_i$, to przyjmiemy, że zachodzi także $A_i'$.
W~przeciwnym przypadku sukces w~$i$\nbhyphen tej próbie ciągu $A'$ będzie generowany z~pewnym prawdopodobieństwem $r_i$.
Mamy
\[
    p_i' = \Pr(A_i') = \Pr(A_i'\cap A_i)+\Pr(A_i'\cap\overline{A_i}) = p_i+(1-p_i)\cdot r_i,
\]
skąd
\[
