...
%W~naszej implementacji drzew AVL użyjemy specjalnego węzła-wartownika.
%Dla drzewa AVL $T$ wartownik \attrib{T}{nil} będzie obiektem o~takich samych polach jak zwykły węzeł w~drzewie $T$.
%Dla uproszczenia poniższych operacji przyjmijmy, że $h$ w~wartowniku ma wartość $-1$, natomiast pozostałe pola mogą mieć dowolne wartości.
%Podobnie jak w~drzewach czerwono-czarnych jeden wartownik $\attrib{T}{nil}$ będzie reprezentował wszystkie \const{nil} -- zarówno dla ojca korzenia drzewa $T$, jak i~dla pustych synów węzłów w~$T$.
