...
\begin{tikzpicture}[
	...
	\node[right=45mm of left root, blackened] (middle root) {2}
		child {node[label=$z$] {1}}
		child {node {3}};
	
	\draw[arrow, shorten >=10mm, shorten <=10mm, dashed, thick] (left root.east) -- node[index node] {$\proc{RB-Insert}(T,z)$} (middle root.west);
	
	\node[right=45mm of middle root, blackened] (right root) {2}
		child[missing]
		child {node {3}};
	
	\draw[arrow, shorten >=10mm, shorten <=10mm, dashed, thick] (middle root.east) -- node[index node] {$\proc{RB-Delete}(T,z)$} (right root.west);
