...
Wykonuje w~tym celu odpowiednie rotacje, przenosząc dzięki temu węzeł $x$ na wyższe poziomy drzepca aż do momentu, gdy $x$ osiągnie korzeń albo gdy własność kopca między $x$ i~\attrib{x}{p} jest już spełniona.
...
\subproblem %13-4(e)
Bezpośrednio po umieszczeniu nowego węzła $x$ w~drzepcu w~wierszu \ref{li:treap-insert-tree-insert} procedury \proc{Treap-Insert}, zarówno lewe, jak i~prawe poddrzewo $x$ są puste -- ich lewy i~prawy kręgosłup mają długość~0.
Każde wykonanie rotacji w~tej procedurze przenosi węzeł $x$ o~jeden poziom wyżej w~drzepcu, zwiększając przy tym o~1 długość $C$ prawego kręgosłupa w~lewym poddrzewie $x$ albo długość $D$ lewego kręgosłupa w~prawym poddrzewie $x$.
Po zakończeniu działania procedury \proc{Treap-Insert}, wartość $C$ jest równa ilości wywołań z~linii \ref{li:treap-insert-left-rotate}, a~$D$ jest równe ilości wywołań z~linii \ref{li:treap-insert-right-rotate}.
