...
Wystarczy zatem bezpośrednio po linii 11 zmniejszyć \attrib{T}{bh} o~1, gdy $x=\attrib{T}{root}$.
...
Można zatem, bez naruszenia własności drzewa wyszukiwań binarnych, umieścić węzeł $x$ między $y$ a~\attrib{y}{p}, czyniąc $T_y$ lewym poddrzewem $x$, a~$T_2$ -- jego prawym poddrzewem.
...
\subproblem %13-2(f)
Prześledźmy działanie algorytmu \proc{RB-Join}, którego pseudokod podaliśmy poniżej.
\begin{codebox}
...
W~takiej reprezentacji po zakończeniu działania algorytmu wskaźniki na synów niektórych węzłów w~$T_1$ pokazują na \attrib{T_1}{nil}, a~niektórych węzłów w~$T_2$ -- na \attrib{T_2}{nil}.
...
Zmodyfikowane wersje tych operacji są tylko nieznacznie bardziej skomplikowane z~powodu konieczności wykrywania przypadków, gdy dany wskaźnik jest \const{nil} i~traktowania tej wartości jako wirtualnego czarnego liścia.
