...
A~zatem wszystkie odniesienia do pól parametru $x$ w~liniach 1, 2, 3, 6, 7, 8, 11, 16, 17, 18, 20, 22, 23, są odniesieniami do pól wartownika \attrib{T}{nil}.
...
Zastanówmy się teraz, czy w~jakiejkolwiek innej linii procedury \proc{RB-Delete-Fixup} pola \attrib{T}{nil} są odczytywane lub aktualizowane.
Procedura ta wywoływana jest tylko wtedy, gdy usuwany węzeł $y$ jest czarny.
Jeśli $y$ jest korzeniem, to do procedury pomocniczej zostaje przekazane $x=\attrib{T}{nil}=\attrib{T}{root}$ i~pętla \kw{while} nie wykonuje się.
...
Jak łatwo też zauważyć, $w$ nie zostaje nigdy zaktualizowane na \attrib{T}{nil} w~trakcie działania procedury.
