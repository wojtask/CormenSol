...
\exercise %13.4-1
Zanim wywołana zostanie procedura \proc{RB-Delete-Fixup}, korzeniem może zostać czerwony węzeł tylko w~przypadku, gdy usuwany jest korzeń drzewa, którego dokładnie jeden z~synów jest czerwony.
...
\begin{figure}[!ht]
	...
	{\sffamily\bfseries\doubledash{(c)}{(e)}} Po wycięciu kolejnych elementów, w~drzewie naruszona zostaje wyłącznie własność 5.
...
Można także sprawdzić, że w~żadnym z~wywołań \proc{Left-Rotate} i~\proc{Right-Rotate} w~procedurze \proc{RB-Delete-Fixup} nie ma odwołań do pól wartownika, ponieważ wszystkie węzły, na których operacje te pracują, są wewnętrznymi węzłami drzewa.
