...
A~zatem kolorowanie węzła \attribb{z}{p}{p} na czerwono odbywa się jedynie w~przypadku, gdy \attrib{z}{p} jest czerwony i~nie jest to korzeń.
Stąd mamy, że węzeł \attribb{z}{p}{p} nie jest wartownikiem \attrib{T}{nil}, więc procedura \proc{RB-Insert-Fixup} nigdy nie przypisze wartownikowi koloru \const{red}.
...
\li								$\proc{RB-No-Parents-Left-Rotate}(T,z,r)$
							\End
\li							$\attrib{p}{color}\gets\const{black}$
\li							$\attrib{r}{color}\gets\const{red}$
\li							$\proc{RB-No-Parents-Right-Rotate}(T,r,\proc{Pop}(S))$ \label{li:no-parents-insert-fixup-left-rotate}
						\End
...
\begin{codebox}
\Procname{$\proc{RB-No-Parents-Left-Rotate}(T,x,p)$}
