...
\note{Linia 3 w~procedurze wykonującej lewą rotację powinna być zastąpiona liniami:}
\bignegskip
\begin{codebox}
\zi	\If $\attrib{y}{left}\ne\attrib{T}{nil}$
...
Będziemy zatem używać nazw \proc{RB-Left-Rotate} i~\proc{RB-Right-Rotate} dla procedur wykonujących rotacje na drzewach czerwono czarnych, a~nazwy \proc{Left-Rotate} i~\proc{Right-Rotate} zarezerwujemy dla operacji rotacji dla drzew wyszukiwań binarnych, które działają identycznie jak \proc{RB-Left-Rotate} i~\proc{RB-Right-Rotate}, w~których zamieniono\/ \attrib{T}{nil} z~wiersza 5 (oraz 3 -- patrz wyżej) na \const{nil}.}
\bignegskip

\exercise %13.2-1
Oto pseudokod prawej rotacji na drzewie czerwono-czarnym:
\begin{codebox}
\Procname{$\proc{RB-Right-Rotate}(T,x)$}
