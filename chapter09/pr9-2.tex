...
Działanie to implementujemy za pomocą pojedynczej instrukcji \kw{if} w~wierszach \ref{li:weighted-median-boundary-case-begin}\nbendash\ref{li:weighted-median-boundary-case-end}.
...
\subproblem %9-2(e)
Niech $p_i=\langle x_i,y_i\rangle$, gdzie $i=1$, 2, \dots, $n$, będą punktami wejściowymi w~2\nbhyphen wymiarowym problemie lokalizacji urzędu pocztowego z~metryką miejską.
...
\]
A~zatem w~celu wyznaczenia optymalnego punktu $p$ wystarczy znaleźć jego współrzędne niezależnie jako rozwiązania 1\nbhyphen wymiarowej wersji problemu.
