\begin{tikzpicture}[
	...
	index node/.append style = {node distance=10mm and 2mm},
...
\begin{tikzpicture}
	...
	\node[cell, above left=7mm and 8mm of arr-1-1] (L) {2};
...
\node[outer, below=of pic a.south, anchor=north] (pic b) {
\begin{tikzpicture}
	\matrix[
		array,
	...
	\draw[ultra thick] (arr-1-1.south west) rectangle (arr-1-30.north east);
	\foreach \x in {4,7,...,28} {
		\draw[ultra thick] (arr-1-\x.south west) -- (arr-1-\x.north west);
	}
	\foreach \x in {1, ..., 30} {
		\node[index node, above=of arr-1-\x] {\x};
	}
	\node[left=of arr-1-1.west] {$A$};
	\node[cell, above left=7mm and 8mm of arr-1-1] (L) {7};
	\node[left=of L.west] {$L$};
	...
	\draw[thick] (arr-1-16.south) + (1mm, 0mm) -- +(2mm, -7mm) node[index node, anchor=50] {$\id{key}$};
	\draw[thick] (arr-1-17.south) -- +(0, -10mm) node[index node, anchor=north] {$\id{next}$};
	\draw[thick] (arr-1-18.south) + (-1mm, 0mm) -- +(-2mm, -7mm) node[index node, anchor=140] {$\id{prev}$};
\end{tikzpicture}
};
