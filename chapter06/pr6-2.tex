...
\subproblem %6-2(a)
$d$\nbhyphen kopiec będziemy reprezentować w~tablicy w~następujący sposób.
Podobnie jak w~reprezentacji tablicowej kopców binarnych tablica $A$ reprezentująca $d$\nbhyphen kopiec będzie mieć atrybuty \attrib{A}{length} oraz \attrib{A}{heap-size}.
...
Łatwo pokazać, że indeks $k$\nbhyphen tego od lewej syna węzła o~indeksie $i$, gdzie $k=1$, 2, \dots, $d$, jest opisany wzorem $d(i-1)+k+1$.
...
Czas działania operacji \proc{Insert} dla $d$\nbhyphen kopca jest tego samego rzędu co czas działania wywołania z~wiersza 3.
