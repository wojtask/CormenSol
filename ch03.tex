...
A function is in $O'$ of some other function if both the function and its negation are in $O$ of that other function. The direction beginning with $f(n) = O'(g(n))$ and $f(n) = \Omega(g(n))$ is easy since $f(n)$ must also be $O(g(n))$. The other direction does not automatically follow since $-f(n)$ is not necessarily $O(g(n))$ when $f(n)$ is.
