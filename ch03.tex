...
\]
Wybierając dowolne $0<c_1\le1/2$ i~$n_0$ równe minimum zbioru $S$, spełniamy pierwsze dwie nierówności z~(\ref{eq:3.1-1}) dla wszystkich $n\ge n_0$ należących do $S$. Identyczny rezultat otrzymujemy w~przypadku przeciwnym, gdy $\max(f(n),g(n))=f(n)$. Ostatnia z~nierówności jest spełniona w~obu przypadkach, jeśli przyjmiemy $c_2=1$. Z~przedstawionej argumentacji wnioskujemy, że $\max(f(n),g(n))=\Theta(f(n)+g(n))$.
...
Dowód przeprowadzony w~odwrotnej kolejności pozwala wykazać implikację w~drugą stronę, gdyż układ~(\ref{eq:3.1-5}) doprowadza do nierówności z~definicji $f(n)=\Theta(g(n))$.
...
\[
	c_1g(n) < f(n) < c_2g(n)
\]
dla wszystkich $n\ge n_0$. Dochodzimy do sprzeczności, bowiem nieprawdą jest, że każde liczby $c_1$ i~$c_2$ spełniają $c_1<c_2$, skąd wnioskujemy, że zbiór $o(g(n))\cap\omega(g(n))$ jest pusty. Dzięki pustości tej klasy, nie ma potrzeby definiowania notacji $\theta$ odpowiadającej $\Theta$ i~analogicznej do $o$ i~$\omega$.
...
\bigskip %korekcja underfull vboxa
...
\exercise %3.2-5
Zachodzi równość $\lg^*\lg n=\lg^*n-1$, w~której prawa strona jest funkcją liniową ze względu na $\lg^*n$, podczas gdy wyrażenie $\lg\lg^*n$ to funkcja logarytmiczna względem $\lg^*n$. Ponieważ dla każdego $h(n)$ z~wyjątkiem funkcji stałej, $\lg h(n)=o(h(n))$, to stąd mamy
\[
	\lg\lg^*n = o(\lg^*\lg n).
\]
	...
	\begin{center}
		\[
			\begin{array}{|lc|lc|lc|} \hline
				...
				g_3 & (n+1)! & g_{13} & n^2 & g_{23} & \sqrt{\lg n} \\ \cline{1-2}\cline{5-6}
				g_4 & n! & g_{14} & 4^{\lg n} & g_{24} & \ln\ln n \\ \hline
				g_5 & e^n & g_{15} & n\lg n & g_{25} & 2^{\lg^*n} \\ \cline{1-2}\cline{5-6}
				g_6 & n\cdot2^n & g_{16} & \lg(n!) & g_{26} & \lg^*n \\ \cline{1-4}
				g_7 & 2^n & g_{17} & n & g_{27} & \lg^*\lg n \\ \cline{1-2}\cline{5-6}
				g_8 & (3/2)^n & g_{18} & 2^{\lg n} & g_{28} & \lg\lg^*n \\ \hline
				g_9 & n^{\lg\lg n} & g_{19} & \sqrt{2}^{\lg n} & g_{29} & n^{1/\!\lg n} \\ \cline{3-4}
				g_{10} & (\lg n)^{\lg n} & g_{20} & 2^{\sqrt{2\lg n}} & g_{30} & 1 \\ \hline
			\end{array}
		\]
	\end{center}
...
\subproblem %3-5(b)
Zaletą nowej notacji jest fakt, że jeśli o~pewnej funkcji $f(n)$ wiemy, że nie jest $O(g(n))$, to jest $\overset{\infty}{\Omega}(g(n))$ (z~poprzedniego punktu), nie istnieją zatem funkcje, których nie da się porównać z~innymi przy pomocy pary notacji $O$ i~$\overset{\infty}{\Omega}$.

Niestety, jeśli $f(n)=\overset{\infty}{\Omega}(g(n))$, to funkcja $f(n)$ niekoniecznie dominuje nad funkcją $g(n)$, gdyż w~nieskończenie wielu punktach może przyjmować wartości mniejsze od $g(n)$. Nieskończenie wiele punktów, o~których mowa w~definicji nowej notacji może też występować daleko ponad maksymalnym rozmiarem danych wejściowych. Wady te sprawiają, że użycie nowej notacji ma słabe zastosowanie praktyczne i~ma większe znaczenie w~teorii.

\subproblem %3-5(c)
W~przypadku implikacji w~lewą stronę, z~warunku $f(n)=\Theta(g(n))$ wynika, że $f(n)\ge0$, a~więc $f$ jest funkcją nieujemną i~twierdzenie stosuje się bez zmian, czyli implikacja zachodzi.
...
Dowód twierdzenia~3.1 dla notacji $\overset{\sim}{O}$, $\overset{\sim}{\Omega}$ i~$\overset{\sim}{\Theta}$ przebiega analogicznie jak dowód jego oryginalnego odpowiednika, przeprowadzony w~\zad{3.1-5}. Z definicji notacji $\overset{\sim}{\Theta}$ mamy, że $f(n)=\overset{\sim}{\Theta}(g(n))$ wtedy i~tylko wtedy, gdy istnieją takie stałe $c_1$, $c_2$, $k_1$, $k_2$,~$n_0>0$, że
\[
...
\[
	...
	\end{cases}
\]
Nie można podać oszacowania dokładnego, bo $f_c^*(n)=\omega(g(n))$ dla dowolnej funkcji $g(n)$.
...
\subproblem %3-6(h)
Wersje iterowane funkcji $f(n)$ są zbyt skomplikowane, aby badać je analogicznie jak w~poprzednich punktach. Dlatego znajdziemy oszacowanie górne $g(n)$ funkcji $f_c^*(n)$ i~to ono będzie przedmiotem naszej analizy.
...
Można znaleźć nieco lepsze oszacowanie $f(n)$ zauważając, że $f_c^*(n)=1$ dla $3\le n\le4$. Przyjmijmy zatem, że $n>4$. Zachodzi wtedy $\log_n\!2<1/2$, a~zatem można ograniczyć $f(n)$ przez $g(n)=n/2$. Kolejne iteracje mają postać $g^{(i)}(n)=n/2^i$. Ponieważ nierówność $n/2^i\le2$ jest spełniona dla $i\ge\lg n-1$, to prawdą jest, że $f_c^*(n)=O(\lg n)$.
