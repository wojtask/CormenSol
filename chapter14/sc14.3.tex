...
\exercise %14.3-1
Procedura \proc{Interval-Left-Rotate} dla drzew przedziałowych działa identycznie jak procedura \proc{Left-Rotate} dla drzew czerwono-czarnych, z~tą różnicą, że tuż przed zakończeniem działania modyfikuje \attrib{x}{max} i~\attrib{y}{max}, gdzie $y$ jest nowym ojcem $x$ po przeprowadzeniu rotacji.
...
\exercise %14.3-4
Poniższy pseudokod implementuje szukany algorytm, wypisując szukane przedziały.
...
Każdy węzeł drzewa $T$ testowany jest w~algorytmie co najwyżej dwa razy -- raz w~linii \ref{li:interval-search-all-overlap} i~raz w~linii \ref{li:interval-search-all-examine-left} albo \ref{li:interval-search-all-examine-right}.
Każdy taki test zajmuje czas stały, dlatego czas działania algorytmu nie przekracza $O(n)$.
Z~drugiej strony, jeśli procedura schodzi rekurencyjnie do któregoś z~poddrzew, to poddrzewo to musi zawierać przedział zachodzący na $i$.
Dlatego czas działania również nie może przekroczyć $O(k\lg n)$, gdyż wysokość drzewa $T$ wynosi $O(\lg n)$, a~liczba przedziałów zachodzących na $i$ w~$T$ wynosi $k$.
...
Niech $y$ będzie pierwszym napotkanym węzłem w~$T$ podczas tego wstawiania, gdzie $\attribb{y}{int}{low}=\attribb{x}{int}{low}$, albo \attrib{T}{nil}, jeżeli węzeł taki nie istnieje.
Wszystkie przedziały w~drzewie $T$ o~takich samych lewych końcach jak lewy koniec \attrib{x}{int} znajdują się w~poddrzewie o~korzeniu w~$y$.
...
Operacja \proc{Interval-Search-Exactly} wyznacza szukany węzeł w~identyczny sposób jak opisane powyżej wyszukiwanie miejsca w~drzewie podczas wstawiania do niego nowego węzła.
