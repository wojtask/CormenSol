...
\exercise{} %1.1-3
\noindent W~następującym zestawieniu porównano listę dwukierunkową ze zwykłą tablicą.
...
\exercise{} %1.1-5
\noindent Problemem, którego pożądanym rozwiązaniem jest rozwiązanie dokładne, jest np. wyszukiwanie w~nieuporządkowanej tablicy. Obecność jednego elementu w~tej tablicy jest na ogół niezależna od obecności innych, zatem po sprawdzeniu kilku z~nich, nie znajdując poszukiwanego elementu, nadal dysponujemy taką samą wiedzą jak przed rozpoczęciem szukania.
...
\exercise{} %1.2-1
\noindent Przykładem aplikacji, w~której wykorzystywane są różnorodne algorytmy jest współczesna gra komputerowa. Jej silnik grafiki trójwymiarowej może być zaawansowanym środowiskiem, w~którym zastosowano algorytmy geometrii obliczeniowej i~renderowania grafiki 3D, jak również wiele algorytmów numerycznych do wyznaczania interpolacji oraz algorytmy grafowe. Także w~dziedzinie sztucznej inteligencji opracowano wiele zaawansowanych algorytmów. Ponadto często spotykane problemy, takie jak wyszukiwanie elementu w~tablicy czy sortowanie są rozwiązywane w~takiej aplikacji.

\exercise{} %1.2-2
\noindent Nierówność $8n^2<64n\lg n$ jest spełniona dla $2\le n\le43$, zatem sortowanie przez wstawianie uporządkuje tablicę o~rozmiarze nie przekraczającym~43 szybciej niż sortowanie przez scalanie.

\exercise{} %1.2-3
\noindent Najmniejszym dodatnim $n$ spełniającym nierówność $100n^2<2^n$ jest $n=14$.

\problems

\exercise{Porównanie czasów działania} %1-1
\noindent W~tabeli~\ref{tab:1-1} zebrano wyznaczone wartości. Dla uproszczenia obliczeń przyjęto, że na każdy miesiąc przypada 30 dni; nie uwzględniono też lat przestępnych.
