...
\end{codebox}
W~celu wykonania permutacji odwracającej bity utrzymujemy 2 \singledash{$k$}{bitowe} liczniki binarne -- zwykły oraz z~odwróconymi bitami -- i~zmieniamy elementy w~tablicy $A$ znajdujące się na indeksach równych tym licznikom, upewniając się, że zamiany te nie powtarzają się.
\begin{codebox}
\Procname{$\proc{Bit-Reversal}'(A)$}
\li	$n\gets\attrib{A}{length}$
\li	$k\gets\lg n$
