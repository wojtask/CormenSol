\section*{Dodatek B: Zbiory i nie tylko}
...
\paragraph{B.3-4.}
% Rozważmy rozwinięcie binarne $m$-bitowej liczby naturalnej $n>0$,
% \[
% 	\mathrm{b}(n) = \beta_{m-1}\dots\beta_1\beta_0,
% \]
% przy czym $\beta_{m-1}=1$ jest jej najbardziej znaczącym bitem. Podzielmy to rozwinięcie (bez $\beta_{m-1}$) na $2$ podciągi, mniej więcej tej samej długości,
% \begin{eqnarray*}
% 	\mathrm{b}_1(n) &=& \beta_{m'}\dots\beta_1\beta_0, \\
% 	\mathrm{b}_2(n) &=& \beta_{m-2}\dots\beta_{m'+2}\beta_{m'+1},
% \end{eqnarray*}
% gdzie $m'=\lfloor (m-2)/2\rfloor$. Potraktujmy $\mathrm{b}_1(n)$ jako rozwinięcie binarne pewnej $m'$-bitowej liczby całkowitej $x_n\ge 0$. Jeśli $x_n=0$, to niech $\mathrm{b}_2(n)$ również będzie rozwinięciem binarnym pewnej $m'$-bitowej liczby całkowitej $y_n-1$. W przeciwnym przypadku, czyli jeśli $x_n$ jest niezerowe, to $\mathrm{b}_2(n)$ traktujemy jako kodowanie U2 pewnej $m'$-bitowej liczby całkowitej $y_n$. Wtedy para uporządkowana $(x_n,y_n)\in \mathbb{N}_+\times\mathbb{Z}\cup\{ 0\}\times\mathbb{N}_+$, gdzie $\mathbb{N}_+$ jest zbiorem liczb naturalnych dodatnich.

%Niech $f(n)=(x_n,y_n)$.

% Opiszemy funkcję
% \[
% 	f\!:\;\mathbb{N}_+\;\mapsto\;\mathbb{N}_+\times\mathbb{Z}\cup\{ 0\}\times\mathbb{N}_+.
% \]

% Niech $\mathrm{b}(n)$ będzie rozwinięciem binarnym $m$-bitowej liczby $|n|\in\mathbb{N}$,
% \[
% 	\mathrm{b}(n) = \beta_{m-1}\beta_{m-2}\dots \beta_1\beta_0,
% \]
% a $B$ -- zbiorem wszystkich możliwych rozwinięć.
% 
% Opiszemy teraz funkcję $p:B\rightarrow\left(\{-1,+1\}\times\mathbb{N}\right)^2$. Dla $b=\mathrm{b}(0)$, $p(b)=(1,0,1,0)$. Dla każdego innego $b=\mathrm{b}(|n|)\in B$, niech $p$ zwraca czwórkę uporządkowaną $(\pi_1,\pi_2,\pi_3,\pi_4)$ taką, że:
% \begin{itemize}
% 	\item $\pi_1=\mathrm{sgn}\;n$,
% 	\item $\pi_2$ jest wartością rozwinięcia bitowego $b'=\beta_{m-1-(m\bmod 2)}\dots\beta_3\beta_1$, czyli taką liczbą naturalną, dla której $\mathrm{b}(\pi_2)=b'$
% 	\item $\pi_3=+1$, gdy $\beta_0=0$ i $\pi_3=-1$ w przeciwnym przypadku,
% 	\item $\pi_4$ jest wartością rozwinięcia bitowego $b''=\beta_{m-2+m\bmod 2}\dots\beta_4\beta_2$, czyli taką liczbą naturalną, dla której $\mathrm{b}(\pi_4)=b''$.
% \end{itemize}
% W uproszczeniu, pierwszym elementem wyniku $p(\mathrm{b}(|n|))$ jest znak liczby $n$, trzeci element jest odpowiednim znakiem w zależności od wartości najmniej znaczącego bitu $b$, a drugi i czwarty są wartościami rozwinięć utworzonych z, odpowiednio, nieparzystych i dodatnich parzystych pozycji rozwinięcia $b$.
% 
% \begin{twierdzenie*}
% Niech $n\in\mathbb{Z}$ oraz $p(\mathrm{b}(|n|))=(\pi_1,\pi_2,\pi_3,\pi_4)$. Wtedy $f:\mathbb{Z}\rightarrow\mathbb{Z}^2$ taka, że $f(n) = (\pi_1\pi_2,\pi_3\pi_4)$, jest bijekcją.
% \end{twierdzenie*}
% \begin{proof}
% Kluczowym faktem pozwalającym na wykazanie wymaganych własności $f$ jest to, że każda liczba naturalna posiada dokładnie jedno rozwinięcie binarne, i odwrotnie -- każde rozwinięcie binarne jednoznacznie determinuje liczbę naturalną, która jest wartością tegoż rozwinięcia. Inaczej, odwzorowanie $\mathrm{b}(n)$ w dziedzinie liczb naturalnych jest bijekcją. Rozszerzmy nasze rozumowanie na niezerowe liczby całkowite i potraktujmy dowolną liczbę $n\in\mathbb{Z}\setminus\{ 0\}$ jako parę $(\mathrm{sgn}\,n,\mathrm{b}(|n|))\in\{-1,+1\}\times B$. Dla każdej liczby całkowitej istnieje dokładnie jedna taka para, i odwrotnie. Ponadto, dla każdego rozwinięcia binarnego można utworzyć dokładnie jedną trójkę $(\pi_2,\beta_0,\pi_4)$, a dla każdej takiej trójki będzie istnieć dokładnie jedno rozwinięcie binarne.
% 
% Wnioskujemy zatem, że dla każdej liczby całkowitej $n$ różnej od $0$, możemy wskazać jedną i jedyną czwórkę $(\pi_1,\pi_2,\pi_3,\pi_4)$ wyznaczoną przez $n$. Ponieważ $\pi_1\in\{ -1,+1\}$ oraz $\pi_2\ge 0$, to mamy tylko jeden sposób utworzenia $\pi_1\pi_2$, a więc $f$ jest injekcją. Para $(\pi_1\pi_2,\pi_3\pi_4)$ przyjmuje wszystkie wartości w $\mathbb{Z}\setminus{(0,0)}$. Wartość $(0,0)$ jest osiągana dla $n=0$.
% \end{proof}

\subsection*{B.4. Grafy}
...
%%zamienic na podpisy rusunków
\paragraph{B.4-5.}
%\begin{figure}[h]
	\begin{center}
		\includegraphics{figb.2}
	\end{center}
%	\caption{Wersja nieskierowana grafu skierowanego z rysunku 5.2a}
%\end{figure}
%\begin{figure}[h]
	\begin{center}
		\includegraphics{figb.3}
	\end{center}
%	\caption{Wersja skierowana grafu nieskierowanego z rysunku 5.2b}
%\end{figure}
...
\paragraph{B.5-4.}
% Nierówność $h\ge\left\lfloor\lg n\right\rfloor$ udowodnimy przez indukcję względem $h$. Dla $h=0$ drzewo posiada tylko jeden węzeł, zatem $n=1$ i nierówność zachodzi.
% 
% Rozważmy tylko Załóżmy, że nierówność jest spełniona dla drzewa o wysokości $h$ i posiadającego $n$ węzłów ($n\le 2^{h+1}-1$). Zwiększmy teraz wysokość drzewa o $1$ dodając pewną liczbę nowych węzłów i uzyskując drzewo o $n'$ węzłach. Wykorzystując założenie indukcyjne dostajemy
% \[
% 	h+1\ge\lfloor\lg n\rfloor+1\ge \left\lfloor\lg \left(2^{h+1}-1\right)\right\rfloor+1\ge\left\lfloor\lg \left(2^{h+2}-1\right)\right\rfloor\ge\left\lfloor\lg n'\right\rfloor.
% \]
% 
% Nierówność jest zatem spełniona dla dowolnych drzew binarnych, przy czym równość zachodzi tylko dla drzew pełnych, w których $n=2^{h+1}-1$.
...
%\begin{figure}[h]
	...
	\end{center}
%	\label{figb.3}
%	\caption{Grafy o cyklach o długościach większych niż $3$.}
%\end{figure}
