...
\li		\Do
			$C[i+1]\gets A[i]\BitXor B[i]\BitXor\id{carry}$ \label{li:bin-add-xor}
...
W~pętli \kw{for} w~wierszach \ref{li:bin-add-for-begin}\nobreakdash--\ref{li:bin-add-for-end} realizowane jest dodawanie poszczególnych bitów liczb nieujemnych $a$ i~$b$ od najmniej do najbardziej znaczącego z~uwzględnieniem przeniesienia. Jest to w~istocie użycie operacji \kw{xor}, czyli alternatywy wykluczającej -- wynik takiej operacji na dwóch bitach jest ich sumą modulo~2. Instrukcja warunkowa z~wiersza~\ref{li:bin-add-if} wykrywa przepełnienie i~odpowiednio ustawia nową wartość bitu \id{carry} także za pomocą operacji logicznych -- \kw{or} i~\kw{and}.
...
Mamy $n-1$ przebiegów zewnętrznej pętli \kw{for}, a~wewnętrzna pętla \kw{for} przebiega przez cały nieposortowany fragment tablicy, szukając jego minimalnego elementu, zatem zarówno pesymistyczny jak i~optymistyczny czas działania algorytmu wynosi
\[
...
\exercise %2.3-1
Na rys.~\ref{fig:2.3-1} przedstawiono działanie algorytmu \proc{Insertion-Sort} dla tablicy $A$.
\begin{figure}[ht]
	\begin{center}
...
\li			$k\gets k+1$
		\End
\end{codebox}
Po zakończeniu wykonywania pętli \kw{while} z~wierszy \ref{li:merge-while-begin}\nobreakdash--\ref{li:merge-while-end} powyższej procedury wszystkie elementy z~co najmniej jednej tablicy $L$ lub $R$ są już w~tablicy $A$, więc w~kolejnych dwóch pętlach \kw{while} realizujemy kopiowanie reszty tablicy $L$ lub $R$ na koniec $A$. Co najwyżej jedna z~tych pętli wykona swój kod więcej niż raz.
...
\[
	...
	\end{cases}
\]
Rozwijając rekurencję, dostajemy
\begin{align*}
...
\exercise %2.3-5
Procedura \proc{Binary-Search} przyjmuje na wejściu posortowaną niemalejąco tablicę $A$, szukaną wartość $v$ i~liczby \id{low} oraz \id{high} wyznaczające pewien \compound{$n$}{elementowy} fragment tablicy $A$, w~którym będzie szukane $v$. Procedura porównuje $v$ z~$A[\id{mid}]$, czyli elementem środkowym zakresu $\id{low}\twodots\id{high}$ i~na podstawie wyniku tego porównania eliminuje z~dalszych rozważań odpowiednią połowę zakresu.

Poniżej przedstawiono wersję rekurencyjną oraz iteracyjną algorytmu wyszukiwania binarnego. W~przypadku odnalezienia wartości $v$ w~tablicy $A$ zwracany jest taki indeks $i$, że $A[i]=v$. Jeśli elementu $v$ nie ma w~tablicy, to wynikiem procedury jest specjalna wartość \const{nil}. Aby wyszukiwać w~całej tablicy $A$, należy wywołać jedną z~tych procedur z~parametrami $\id{low}=1$ oraz $\id{high}=\id{length}[A]$.
...
W~obu wersjach algorytmu \proc{Binary-Search} $v$ jest przyrównywane do środkowego elementu zakresu, odrzucana jest połowa zakresu i~$v$ jest poszukiwane w~drugiej połowie. Procedury kończą swe działanie, znajdując $v$ na pewnej pozycji w~tablicy $A$ albo dochodząc do pustego zakresu poszukiwań (czyli $\id{low}>\id{high}$), co oznacza, że elementu $v$ nie ma w~tablicy. Rekurencja opisująca czas działania procedur ma postać
\[
	T(n) =
	\begin{cases}
...
\subproblem %2-1(b)
Uogólniając procedurę scalania dwóch podlist na scalanie $n/k$ podlist w~jedną, można osiągnąć czas $\Theta(n^2\!/k)$ -- kopiujemy $n$ elementów, dla każdego sprawdzając, która z~$n/k$ podlist jest tą, w~której dany element powinien się znaleźć.
...
\end{quote}
\begin{description}
	\item[Inicjowanie:] Przed pierwszą iteracją $i=1$, więc podtablica $A[1\twodots i-1]$ jest pusta, więc oczywiście jest posortowana niemalejąco.
	\item[Utrzymanie:] Z~założenia, że podtablica $A[1\twodots i-1]$ jest posortowana niemalejąco wynika, że $A[i-1]$ jest największym elementem tej podtablicy. Wewnętrzna pętla \kw{for} wyszukuje w~podtablicy $A[i\twodots n]$ najmniejszy element i~umieszcza go na pozycji $i$ (dowód w~poprzednim punkcie). W~podtablicy $A[i\twodots n]$ nie ma mniejszych elementów od $A[i-1]$, a~zatem w~szczególności zachodzi $A[i-1]\le A[i]$. Na podstawie założenia wnioskujemy, że podtablica $A[1\twodots i]$ jest posortowana niemalejąco i~po aktualizacji $i$ niezmiennik zostaje odtworzony.
...
\subproblem %2-3(b)
Niech $A=\langle a_0,a_1,\dots,a_n\rangle$ będzie ciągiem kolejnych współczynników wielomianu $P$. Następujący algorytm oblicza wartość $P(x)$:
\begin{codebox}
