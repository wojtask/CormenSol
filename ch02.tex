...
W~przypadku pesymistycznym procedura sprawdza wszystkie $n$ elementów, nie znajdując szukanego, a~więc otrzymujemy ten sam wynik: $\Theta(n)$.
...
Zatem pesymistyczny czas algorytmu \proc{Sum-Search} wynosi
\[
...
Wewnętrzna pętla \kw{for} wyszukuje w~podtablicy $A[i\twodots\attrib{A}{length}]$ najmniejszy element i~umieszcza go na pozycji $i$ (dowód w~poprzednim punkcie).
