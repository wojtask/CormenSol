...
W~procedurze \proc{Insertion-Sort} pętla \kw{while} w~wierszach \doubledash{5}{7} przenosi element znajdujący się początkowo na pozycji $j$ o~$\mathrm{inv}(j)$ pozycji w~lewo, dzięki czemu wyeliminowanych zostaje $\mathrm{inv}(j)$ inwersji.
