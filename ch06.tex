...
\li	\While $k\le d$ i~$\id{child}\le\id{heap-size}[A]$ \label{li:multiary-max-heapify-while-begin}
\li		\Do
...
\li	$\id{heap-size}[A]\gets\id{heap-size}[A]+1$
\li	$A[\id{heap-size}[A]]\gets-\infty$
\li	$\proc{Multiary-Heap-Increase-Key}(A,d,\id{heap-size}[A],\id{key})$ \label{li:multiary-max-heap-insert-increase-key}
\end{codebox}
