\begin{tikzpicture}[
	...
	outer/.append style = {node distance=10mm and 8mm, scale=0.7},
	inner/.style = {draw=none, fill=none, node distance=4mm},
	on grid
]
...
\begin{tikzpicture}
	\node[inner] {
	\begin{tikzpicture}
		\matrix[array, row 1/.style = {column sep=0pt, nodes={fill=none, draw=none}}] { & & & & & & & & & & \\};
...
\node[outer, below=of pic a.south] (pic d) {
\begin{tikzpicture}
	\node[inner] (pic d1) {
	\begin{tikzpicture}[
...
\node[outer, right=of pic d.east] (pic e) {
\begin{tikzpicture}
	\node[inner] (pic e1) {
	\begin{tikzpicture}[
