...
Oznacza to, że po wykonaniu pętli zachodzi $i=r-1$ i~procedura zwraca $q=r$.

Odpowiedniej modyfikacji procedury dokonujemy poprzez dodanie do ciała pętli \kw{for} zliczania elementów w~podtablicy $A[p\twodots r-1]$ równych elementowi rozdzielającemu $A[r]$.
Jeśli po zakończeniu wykonywania pętli naliczono ich $r-p$, oznacza to, że wszystkie elementy w~$A[p\twodots r]$ są jednakowe i~należy zwrócić $q=\lfloor(p+r)/2\rfloor$.
