...
Wywołujemy następnie nasz algorytm rekurencyjnie w~celu znalezienia \singledash{$i$}{tej} statystyki pozycyjnej podtablicy $A[p+m\twodots r]$.
W~wywołaniu rekurencyjnym stosujemy jednak zmodyfikowaną wersję procedury partycjonującej wykorzystywanej przez algorytm \proc{Select}.
...
Wywołanie rekurencyjne wprowadza składnik $U_i(\lceil n/2\rceil)$, zaś algorytm \proc{Select} wywołany na tablicy $B$ wykonuje $T(2i)$ porównań.
Stąd otrzymujemy
\[
...
Pokazaliśmy, że $c$ istotnie jest stałą, a~zatem podstawa indukcji zachodzi.
...
\subproblem %9-3(c)
Ponieważ $i$ jest stałą, to $T(2i)$ również można potraktować jako wartość stałą.
